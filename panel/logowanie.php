  ...
  <hr class="my-2">
		...
		<?php
			if ($printable) {
				...
				$stm = $pdo->query("SELECT count(*) FROM `uzytkownicy` WHERE `nick` = '{$_POST['login']}' AND `haslo` = '{$_POST['pass']}';");
				$ret = $stm->fetch();
				if ($ret[0] == 1)
					header('Location: panel.php'); 
				else {
					$stm = $pdo->query("SELECT count(*) FROM `uzytkownicy` WHERE `email` LIKE '{$_POST['login']}%' AND `haslo` = '{$_POST['pass']}';");
					$ret = $stm->fetch();
					print_r($ret);
					if ($ret[0] == 1)
						header('Location: panel.php'); 
					else
						print("<p>Brak konta bądź autoryzacja niejednoznaczna!</p>");
				}
			}
		?>
	</body>
